...
export function ColorPalette() {
  ...
  return (
    <div className="relative">
      <button 
        className="flex justify-center items-center w-8 h-8 rounded-md hover:bg-black/10 dark:hover:bg-white/10 transition-all hover:scale-105 active:scale-95"
        onClick={() => setShowPalette(!showPalette)}
        title="Color & Brush"
      >
        <div className="w-4 h-4 rounded-full border border-black/20 dark:border-white/50 shadow-sm transition-colors" style={{ backgroundColor: color }} />
      </button>
      
      {showPalette && (
        <div className="absolute top-10 right-0 bg-white/95 dark:bg-gray-800/95 backdrop-blur-xl border border-black/10 dark:border-white/10 rounded-xl shadow-2xl p-4 flex flex-col gap-4 z-50 animate-scale-in origin-top-right w-56">
